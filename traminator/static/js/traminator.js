...
$(function () {
    ...
    function TraminatorViewModel(parameters) {
        ...
        self.tramOffsets = [ko.observable(), ko.observable(), ko.observable(), ko.observable()];
        ...
        self.probe = function (location) {
            ...
            OctoPrint.simpleApiCommand("traminator", "probe", { "location": location }).done(function (response) {
                console.log(response);
            });
        ...
        self.ublAutoProbe = function () {
            OctoPrint.simpleApiCommand("traminator", "ublAutoProbe").done(function (response) {
                console.log(response);
            });
        }

        self.ublFillUnpopulated = function () {
            OctoPrint.simpleApiCommand("traminator", "ublFillUnpopulated").done(function (response) {
                console.log(response);
            });
        }
        ...
        self.onDataUpdaterPluginMessage = function (plugin, data) {
            ...
            if (data.type == "meshProbingProgress") {
                ...
                self.ublAutoProbeProgressPercent(parseFloat(current) / parseFloat(total) * 100)
            }
        }
    }
